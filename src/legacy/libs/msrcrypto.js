...
var msrCrypto = msrCrypto || (function () {
    ...
    var msrcryptoGcm = function (/*@type(msrcryptoAes)*/ blockCipher) {
        ...
        function decrypt(/*@type(Array)*/cipherBytes, /*@type(Array)*/tagBytes) {
            ...
            else if (utils.arraysEqual(t, tagBytes)) {
                return p;
            } else {
                return null;
            }
    ...
    if (typeof operations !== "undefined") {
        ...
        msrcryptoGcm.decrypt = function (p) {
            ...
            var tagLength, cipherBytes, tagBytes;
            if (p.algorithm.tagLength) {
                tagLength = Math.floor(p.algorithm.tagLength / 8);
                cipherBytes = p.buffer.slice(0, p.buffer.length - tagLength);
                tagBytes = p.buffer.slice(-tagLength);
            }
            else {
                cipherBytes = p.buffer;
                tagBytes = [];
            }
